...
const iobroker = {
  ...
  methods: {
    ...
    async saveAdapterConfig() {
      await this.setIobrokerConfig(this.iobrokerConfig);
    ...
    closeAdapterConfig() {
      return (this.iobrokerConfigChanged ?
        this.$confirm('okColor=error darken-2|Really exit without saving?') :
        Promise.resolve(true)
      ).then((res) => this.$alert('close now ' + res));
    },

    async getObject(id) {
      return this.socketEmit("getObject", id).then(res => res, e => (console.log(e), null));
    },

    async getState(id) {
      return this.socketEmit("getState", id).then(res => res, e => (console.log(e), null));
    },
    ...
    async getExtendableInstances(adapter) {
      ...
      return this.socketEmit("getObjectView", "system", "instance", null)
        .then(doc => {
              var res = [];
              for (var i = 0; i < doc.rows.length; i++) 
    ...
    async loadSystemConfig() {
      let res = await this.socketEmit("getObject", "system.config").then(x => x, e => (console.log(e), null));
      if (res && res.common) {
        this.ioBrokerSystemConfig = res;
        this.iobrokerLang = res.common.language || this.iobrokerLang;
      }
      res = await this.socketEmit("getObject", "system.certificates").then(x => x, e => (console.log(e), null));
      if (res && res.native && res.native.certificates) {
        ...
        for (var c in res.native.certificates) {
          ...
          var _cert = {
            name: c,
            type: res.native.certificates[c].substring(
                0,
